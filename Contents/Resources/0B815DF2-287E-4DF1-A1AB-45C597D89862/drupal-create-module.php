...
if (!empty($module_name)) {
  $module_slug = slug($module_name, 20, '_');
  echo sprintf('mkdir "%s/%s"', $dirname, $module_slug);
  shell_exec(sprintf('mkdir "%s/%s"', $dirname, $module_slug));
  shell_exec(sprintf('touch "%s/%s/%s.info"', $dirname, $module_slug, $module_slug));
  shell_exec(sprintf('touch "%s/%s/%s.install"', $dirname, $module_slug, $module_slug));
  shell_exec(sprintf('touch "%s/%s/%s.module"', $dirname, $module_slug, $module_slug));
  
  $module_info = sprintf("; \$Id\$
name = %s
...
; - http://drupal.org/node/231036 (Drupal 6)", $module_name);
  
  chdir(sprintf('%s/%s', $dirname, $module_slug));
  $fh = fopen(sprintf('%s.info', $module_slug), 'w');
  fwrite($fh, $module_info);
  fclose($fh);
}
